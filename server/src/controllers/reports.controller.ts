...
export const create: RequestHandler = async (req, res) => {
  ...
  if (targetAuthor.equals(me._id)) throw new AppError(400, "You can't report your own content");

  try {
    await Report.create({ reporter: me._id, targetType, target: targetId, post: post._id, targetAuthor, reason, details: details ?? '' });
  } catch (err) {
